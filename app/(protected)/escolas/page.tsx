...
export default function EscolasPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await institutionService.createInstitution(Number(beneficiaryId), sections, answers);
      setSuccess(true);
    } catch (err) {
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Box sx={{ mb: 3 }}>
          <TextField
            ...
            fullWidth
            inputProps={{ min: 1 }}
            helperText="Informe o ID do aluno/beneficiário vinculado a esta escola"
          />
        </Box>
        ...
        <Box sx={{ mt: 3, display: 'flex', gap: 2 }}>
          <Button
            ...
            disabled={submitting}
          >
            {submitting ? 'Salvando...' : 'Salvar Escola'}
          </Button>
          <Button
            variant="outlined"
            fullWidth
